...
if __name__ == '__main__':
    ...
    epochs = 100
    is_pretrained = False
    train_enhance_pipeline = ['gamma_correction']
    eval_enhance_pipeline = ['gamma_correction']
    ...
    include_top = True
    include_fc = True
    model = resnet50(num_classes, include_top, include_fc)
    if is_pretrained:
        model_weights_dir = './pre_trained_models/resnet50-19c8e357.pth'
        # load pre-trained model
        pretrained_weights = torch.load(model_weights_dir)
        model_weights = model.state_dict()
        filtered_weights = {k: v for k, v in pretrained_weights.items() if k in model_weights
                            and model_weights[k].size() == v.size()}
    ...
    print_log(summary(model, input_size=(batch_size, 3, 224, 224), verbose=0), log=log_filename)
    print_log(f'if pretrained: {is_pretrained}', log=log_filename)
    ...
    saved_model_dir = "./saved_models/resnet_lf/"
    if not os.path.exists(saved_model_dir):
        os.makedirs(saved_model_dir)
    saved_model_path = os.path.join(saved_model_dir, f"resnet_lf_{now}.pth")
    if is_pretrained:
        saved_model_path = os.path.join(saved_model_dir, f"resnet_pretrained_lf_{now}.pth")
